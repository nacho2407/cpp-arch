...
namespace nacho
{
        ...
        ThreadPool::ThreadPool(std::size_t s): pool_size_(s), is_disabled_(false)
        {
                // workers_ 공간 확보
                workers_.reserve(s);
        ...
        void ThreadPool::worker(void)
        {
                while(true) {
                        ...
                        jobs_cv_.wait(lk, [this]() {
                                return (!this->jobs_.empty() || this->is_disabled_);
                        });

                        // 남은 작업이 없고 ThreadPool이 사용 정지된 상태이면 쓰레드 종료
                        if(this->jobs_.empty() && is_disabled_)
                                return;

                        // jobs에서 수행할 작업 꺼내기
                        std::function<void(void)> job = jobs_.front();
                        jobs_.pop();
