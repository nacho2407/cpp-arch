...
&nbsp;C++ 이후의 언어들은 대부분 GC를 탑재한 채로 출시되어 더 이상 사용하지 않는 메모리 공간을 쉽게 해재시켜주고 프로그래머들의 책임을 크게 덜어주었는데, C/C++은 그런 기능이 없었다. 모던 C++에서는 **스마트 포인터**로 인해 그러한 부담을 크게 줄여주었다.
...
&nbsp;이 문제는 `std::make_shared`를 이용하더라도 한 번은 짚고 넘어가야하는 부분인데, 왜냐하면 **클래스 내부에서 자신의 `std::shared_ptr`를 반환하는 함수를 구현**하려 할 때, `std::shared_ptr<dorito>(this)`와 같이 구현하면 의도치 않게 해당 힙 공간이 해제되어버릴 수도 있기 때문이다.
...
{
        return shared_from_this(); // 자신의 std::shared_ptr을 반환
}
```
